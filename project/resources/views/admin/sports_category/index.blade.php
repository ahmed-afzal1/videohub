...
@section('content')
<input type="hidden" id="headerdata" value="{{ __('SPORTS CATEGORY') }}">
<div class="container-fluid" id="container-wrapper">
  <div class="d-sm-flex align-items-center justify-content-between mb-4">
      <h1 class="h3 mb-0 text-gray-800">{{ __('Sports Category') }}</h1>
      <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}">{{ __('Dashboaard') }}</a></li>
          <li class="breadcrumb-item active" aria-current="page">{{ __('Sports Category') }}</li>
      </ol>
  </div>
...
{{-- DATA TABLE --}}
    <script type="text/javascript">
		var table = $('#geniustable').DataTable({
               ...
               processing: true,
               serverSide: true,
               ajax: '{{ route('admin-sports-cat-datatables') }}',
               columns: [
                        { data: 'name', name: 'name' },
        ...
        '<a class="btn btn-primary " href="javascript:;" data-href="{{route('admin-sports-cat-create')}}" id="add-data" data-toggle="modal" data-target="#modal1">'+
        '<i class="fas fa-plus"></i> {{ __('Add New Sports Category') }}'+
        '</a>'+
        '</div>');
      });
{{-- DATA TABLE ENDS--}}
